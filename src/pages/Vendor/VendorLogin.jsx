...
const VendorLogin = () => {
  ...
  const handleRegister = async () => {
    clear()
    if (!shopName||!email||!password||!confirmPwd) { setErr("Fill all fields"); return }
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", fontFamily:FONT }}>
      ...
      {/* LEFT PANEL */}
      <div style={{ flex:1, background:`linear-gradient(135deg,${BLUE} 0%,${BLUE2} 100%)`, display:"flex", flexDirection:"column", justifyContent:"center", padding:"60px 48px", color:WHITE }} className="left-panel">
        ...
        <div style={{ width:52,height:52,background:"rgba(255,255,255,0.2)",borderRadius:16,display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,marginBottom:24 }}>🏪</div>
        <h2 style={{ fontSize:32,fontWeight:900,margin:"0 0 14px",letterSpacing:-0.5,lineHeight:1.2 }}>Vendor Portal</h2>
        <p style={{ opacity:0.8,fontSize:15,margin:"0 0 32px",lineHeight:1.8,maxWidth:320 }}>
          Update your store prices in seconds. Customers in your area see them instantly on the map.
        </p>
        {["Update prices from any phone","Customers find you on the map","Build trust with price history","Free — no subscription ever"].map(p=>(
          <div key={p} style={{ display:"flex",alignItems:"center",gap:12,marginBottom:12,fontSize:14,opacity:0.9 }}>
            <div style={{ width:22,height:22,background:"rgba(255,255,255,0.2)",borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:900,flexShrink:0 }}>✓</div>
            {p}
          </div>
        ))}
      </div>
      ...
      {/* RIGHT FORM */}
      <div style={{ width:"min(100%,480px)",background:"#F9FAFB",display:"flex",alignItems:"center",justifyContent:"center",padding:"40px 32px" }}>
        <div style={{ width:"100%",maxWidth:360 }}>
          ...
          {err  && <div style={{ background:"#FEE2E2",border:"1px solid #FECACA",borderRadius:10,padding:"10px 14px",color:"#991B1B",fontSize:13,fontWeight:600,marginBottom:16 }}>⚠️ {err}</div>}
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
            ...
            {!isLogin && (
              <div>
                <label style={{ display:"block",color:GRAY,fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:6 }}>Shop Name</label>
                <input placeholder="e.g. Sharma Kirana Store" value={shopName} onChange={e=>setShopName(e.target.value)} style={inputStyle}
            ...
            <div>
              ...
              <input type="email" placeholder="vendor@example.com" value={email} onChange={e=>{ setEmail(e.target.value); clear() }} style={inputStyle}
                onFocus={e=>e.target.style.borderColor=BLUE} onBlur={e=>e.target.style.borderColor="#E5E7EB"} />
            </div>
            ...
            <div>
              ...
              <div style={{ position:"relative" }}>
                <input type={showPwd?"text":"password"} placeholder={isLogin?"••••••••":"Minimum 6 characters"} value={password}
                  onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==="Enter"&&isLogin&&handleLogin()}
            ...
            {!isLogin && (
              <div>
                ...
                <div style={{ position:"relative" }}>
                  <input type={showConfirm?"text":"password"} placeholder="Re-enter password" value={confirmPwd} onChange={e=>setConfirmPwd(e.target.value)}
                    style={{ ...inputStyle,paddingRight:44 }}
                    onFocus={e=>e.target.style.borderColor=BLUE} onBlur={e=>e.target.style.borderColor="#E5E7EB"} />
                  <button onClick={()=>setShowConfirm(!showConfirm)} style={{ position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:GRAY,fontSize:16,padding:0 }}>
                    {showConfirm?"🙈":"👁️"}
                  </button>
                </div>
                {confirmPwd && <div style={{ marginTop:5,fontSize:11,fontWeight:600,color:password===confirmPwd?"#16A34A":"#DC2626" }}>{password===confirmPwd?"✓ Passwords match":"✗ Passwords do not match"}</div>}
              </div>
            )}
            ...
            {isLogin && (
              <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between" }}>
                <label style={{ display:"flex",alignItems:"center",gap:8,cursor:"pointer" }}>
                  <div onClick={()=>setRememberMe(!rememberMe)} style={{ width:18,height:18,borderRadius:5,border:`2px solid ${rememberMe?BLUE:"#D1D5DB"}`,background:rememberMe?BLUE:WHITE,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",transition:"all 0.2s",flexShrink:0 }}>
                    {rememberMe && <span style={{ color:WHITE,fontSize:10,fontWeight:900 }}>✓</span>}
                  </div>
                  <span style={{ color:GRAY,fontSize:13 }}>Remember me</span>
                </label>
                <button onClick={handleForgotPassword} style={{ background:"none",border:"none",color:BLUE,fontSize:13,fontWeight:700,cursor:"pointer",padding:0,fontFamily:FONT }}>
                  Forgot Password?
                </button>
              </div>
            )}
          ...
          <div style={{ textAlign:"center",marginTop:20,fontSize:13,color:GRAY }}>
            {isLogin ? <>Don't have an account?{" "}<button onClick={()=>{ setMode("register"); clear() }} style={{ background:"none",border:"none",color:BLUE,fontWeight:700,cursor:"pointer",fontSize:13,fontFamily:FONT }}>Register here</button></>
